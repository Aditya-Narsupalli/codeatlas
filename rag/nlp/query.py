...
# CodeAtlas Phase 4 — identifier-splitting preprocessor.
# Import is lazy-safe: if the codeatlas package is absent the hook is skipped.
try:
    from rag.retrieval.code_search import preprocess_query as _ca_preprocess_query
    _CA_PREPROCESSOR_AVAILABLE = True
except ImportError:  # pragma: no cover
    _CA_PREPROCESSOR_AVAILABLE = False
...
class FulltextQueryer(QueryBase):
    ...
    def question(self, txt, tbl="qa", min_match: float = 0.6):
        # CodeAtlas Phase 4 — pre-retrieval identifier splitting.
        # When code_search flag is ON, camelCase/snake_case tokens in `txt`
        # are expanded into space-separated words before BM25 sees them.
        # When flag is OFF this is a single boolean check with no other cost.
        if _CA_PREPROCESSOR_AVAILABLE:
            txt = _ca_preprocess_query(txt)
        original_query = txt
